...
Analysis:
t = n(n+1)/2 => n**2 + n -2t = 0. This is a quadratic function with two solutions. We know that t, n > 0 so we can disregard one solution which means that in order to check if a word is a triangle number we need to check

    n = (math.sqrt(1 + 8*t) - 1) / 2 , t is the word value
...
def compute():
    ...
    for i in range(len(WORDS)):
        wordsum = (math.sqrt(1 + 8*word_value(WORDS[i])) - 1) / 2
...
def word_value(word):
    res = 0
    for w in word:
        res += (ord(w) + 1 - 65)    # because ord('A') = 65
    return res
